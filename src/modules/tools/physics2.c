...
void       dguard2_new(dguard2_t* dguard, float delta);
...
void physics2_set_gravity(vec_t* masses, v2_t gravity, float delta);
void physics2_set_distances(vec_t* dguards, float delta);
void physics2_set_angles(vec_t* aguards);
void physics2_set_positions(vec_t* masses, surfaces_t* surfaces, float delta);
void physics2_new_mass_position(mass2_t* mass, surfaces_t* surfaces, float delta);
...
void dguard2_new(dguard2_t* dguard, float ratio)
{
...
void physics2_new_mass_position(mass2_t* mass, surfaces_t* surfaces, float delta)
{
    v2_t massbasis = v2_scale(mass->basis, delta);
    v2_t masstrans = mass->trans;

    // we have to store actual inertia with every collosion fragment
    segment2_t inertia = {.trans = masstrans, .basis = massbasis};
	...
	else if (collision.count == 1)
	{
	    ...
	    massbasis   = v2_scale(v2_mirror(collision.segments[0].basis, massbasis), mass->elasticity);
	...
	else if (collision.count > 1)
	{
	    ...
	    if (v2_length(massbasis) <= mass->radius)
	    {
		inertia.basis = v2_init(0.0, 0.0);
		break;
	    }
...
void physics2_set_gravity(vec_t* masses, v2_t gravity, float delta)
{
	...
	mass->basis   = v2_add(mass->basis, v2_scale(gravity, delta));
    }
}
...
void physics2_set_positions(vec_t* masses, surfaces_t* surfaces, float delta)
{
	...
	physics2_new_mass_position(mass, surfaces, delta);
    }
}
