...
bm_rgba_t* gfx_bm_rgba_from_grayscale(int the_width, int the_height, uint32_t backcolor, uint32_t fontcolor, unsigned char* the_bitmap);
...
bm_rgba_t* gfx_bm_rgba_from_grayscale(int the_width, int the_height, uint32_t backcolor, uint32_t fontcolor, unsigned char* the_bitmap)
{
    ...
    bm_rgba_t* bitmap = bm_rgba_new(the_width, the_height);

    for (int index = 0; index < the_width * the_height; index++)
    {
	float ratio                 = (float) the_bitmap[index] / 255.0;
	bitmap->data[index * 4]     = (int) (fr * ratio + br * (1.0 - ratio));
	bitmap->data[index * 4 + 1] = (int) (fg * ratio + bg * (1.0 - ratio));
	bitmap->data[index * 4 + 2] = (int) (fb * ratio + bb * (1.0 - ratio));
	bitmap->data[index * 4 + 3] = (int) (fa * ratio + ba * (1.0 - ratio));
    }
